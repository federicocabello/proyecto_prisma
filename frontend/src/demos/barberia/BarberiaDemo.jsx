...
import BarberiaHeader from "./components/BarberiaHeader";
import BarberiaStats from "./components/BarberiaStats";
import BarberiaCalendar from "./components/BarberiaCalendar";
import BarberiaAppointments from "./components/BarberiaAppointments";
import BarberiaServices from "./components/BarberiaServices";
    ...
    <main className="min-h-screen bg-slate-100">
      <aside className="fixed left-0 top-0 hidden h-screen w-72 border-r border-slate-200 bg-[#07111F] p-6 text-white lg:block">
        <Link to="/" className="block">
          ...
          <p className="mt-1 text-sm text-slate-400">Demo Barbería</p>
        </Link>

        <nav className="mt-10 space-y-2">
          {["Dashboard", "Agenda", "Turnos", "Servicios", "Clientes", "Ingresos"].map(
            (item) => (
              <a
                key={item}
                href="#"
        ...
        <div className="absolute bottom-6 left-6 right-6 rounded-2xl bg-white/10 p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-400">
            Esta pantalla muestra cómo una barbería puede organizar turnos,
            servicios, barberos e ingresos diarios.
          </p>
        </div>
      </aside>
